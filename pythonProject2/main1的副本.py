...
system_state_lock = 3  # 标志系统状态的量 0表示无子线程在运行 1表示正在刷脸 2表示正在录入新面孔。
# 相当于mutex锁，用于线程同步


def usr_log_in():
    # 输入框获取用户名密码
    usr_name = entry_usr_name.get()
    usr_pwd = entry_usr_pwd.get()
    global system_state_lock
    global z
    # open后面的参数是你想要获得文件行数的那个文件（目标文件）
    with open("账号密码.txt", 'rb') as f:
        lines = f.readlines()
    for z in range(len(lines)):
        account = linecache.getline('账号密码.txt',z+1)
        if usr_name == account[:12]:
            if usr_pwd == account[12:-1]:
                print("账号密码验证成功")
                var.set("账号密码验证成功")
                system_state_lock = 0
                ...
                print("密码错误，请重新输入正确的密码!")
                var.set("密码错误，请重新输入正确的密码!")
                entry_usr_pwd.delete('0', 'end')
...
"""    if usr_name == "202014580228" and usr_pwd == "123456":
        print("账号密码验证成功")
        var.set("账号密码验证成功")
        system_state_lock = 0
        entry_usr_pwd.delete('0', 'end')

    elif usr_name == "202014580228" and usr_pwd != "123456":
        print("密码错误，请重新输入正确的密码!")
        var.set("密码错误，请重新输入正确的密码!")
        entry_usr_pwd.delete('0', 'end')
        entry_usr_pwd.focus_set()
...
def Get_new_face():
    ...
    pictur_num = 100  # 表示摄像头拍摄取样的数量,越多效果越好，但获取以及训练的越慢
...
def scan_face():
    ...
    ave_poss = 0
    for times in range(15):  # 每个识别器扫描十遍
        times += 1
        cur_poss = 0
        global success
        global img
...
def f_scan_face_thread():
    ...
    if system_state_lock == 3:
        ...
        if ans == 0:
            ...
            print(ans_name)
            var.set(ans_name)
            webbrowser.open('http://xjw1.ncst.edu.cn/login')
...
entry_usr_name = tk.Entry(window,font=('Arial', 12), width=25)#账号输入
entry_usr_name.place(x=550,y=160)
entry_usr_pwd = tk.Entry(window,font=('Arial', 12), width=25)
entry_usr_pwd.place(x=550,y=200)
button_d = tk.Button(window, text='账号密码核对', font=('Arial', 12), width=10, height=2, command=usr_log_in)
button_d.place(x=550, y=240)
